...
function getWordCounts(content) {
    ...
    // go through each word in the file content
    for (const word of words) {
        if (wordCount[word]) {
            // if word already exists in wordCount, increment the count by 1
            wordCount[word] += 1; 
        } else {
            // if word is not in wordCount, count is 1
            wordCount[word] = 1;
        }
    }
    return wordCount;
...
function printColoredLines(content, wordCount) {
    ...
    for (const line of lines) {
        const coloredLine = line.split(/\W+/).map(word => {
            ...
            // convert to lowercase to match keys in wordCount
            const lowerWord = word.toLowerCase(); 
            // if the word exists in wordCount, color it to it's desired color, else return the original word
